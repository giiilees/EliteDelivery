...
function AddressInput2({ Update }) {
  ...
  const onChangeAddress = (autocomplete) => {
    ...
    // const fullAddress = addressComponents.map(component => component.long_name).join(", ");
    const fullAddress = searchInput.current.value;
    setTextAddress(fullAddress);
    Update("adresseD",  {...extractAddress(place) , name: fullAddress})
  }
  ...
  const reverseGeocode = ({ latitude: lat, longitude: lng}) => {
    ...
    fetch(url)
        ...
        .then(location => {
          ...
          const fullAddress = searchInput.current.value;
          setTextAddress(fullAddress);
          Update("adresseD", {..._address , name: fullAddress});
  ...
  return (
    ...
    <div className='SearchContainer'>
        <div className="search">
          <span><Search /></span>
          <input ref={searchInput} type="text" placeholder="Point de départ...."/>
