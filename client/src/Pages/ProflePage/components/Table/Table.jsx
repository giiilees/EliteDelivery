...
export default function BasicTable() {
  ...
  const [declineBool, setdeclineBool] = useState(false);
  ...
  const GetCommandes = async () => {
    await fetch(`http://localhost:3001/AllCommandes`, {
      method: "GET",
    })
      .then(async (data) => {
        const dataCmd = await data.json();
    ...
    if (Produit) {
      const interval = setInterval(() => {
        setCurrentImageIndex(
          (currentImageIndex) =>
            (currentImageIndex + 1) % Produit.picturesPath.length
        );
      }, 10000);
                  ...
                  <>
                    ...
                    >
                      ...
                      >
                        <div class="infosSuivie">
                          <Tooltip title="Double-clique pour visualiser">
                            <div
                              onDoubleClick={() => {
                                window.open(
                                  `http://localhost:3001/assets/${Produit.picturesPath[currentImageIndex]}`,
                                  "_blank"
                                );
                              }}
                              class="imageSuivie"
                        ...
                        {declineBool ? (
                          <div class="request">
                            <Typography> Motif de refus : </Typography>
                            <TextField
                              label="Nom"
                              onChange={(e) => {
                          ...
                          <div class="request">
                            <Tooltip title="Approuver ?" placement="top">
                              <IconButton
                                onClick={() => {
                                  ChangeStatus(index, "Approuvé");
                                }}
                              >
                                <ThumbUpAlt color="primary" />
                              </IconButton>
                            </Tooltip>
                            <Tooltip title="refuser ?" placement="top">
                              <IconButton
                                onClick={() => {
                                  setdeclineBool(true);
                                }}
                            ...
                            {Produit.Etat == "expertisée" ||
                              Produit.Etat == "en route" ||
                              (Produit.Etat == "Livré" && (
                                <>
                                  <Tooltip
                                    onClick={() => {
                                      ChangeStatus(index, "En route");
                                    }}
                                    title="En route ?"
                                    placement="top"
                                  >
                                    <IconButton>
                                      <DeliveryDining color="info" />
                                    </IconButton>
                                  </Tooltip>

                                  <Tooltip
                                    onClick={() => {
                                      ChangeStatus(index, "Livré");
                                    }}
                                    title="Terminée ?"
                                    placement="top"
                                  >
